...
def prepare_ml_data(df):
    ...
    exclude_cols = ['target', 'Close', 'Open', 'High', 'Low', 'Adj Close', 'returns', 'log_returns']
...
def cross_validate_model(model, X, y, n_splits=5):
    ...
    print(f"  Cross-validation: {mean_score:.3f} ± {std_score:.3f}")
...
def train_random_forest(X_train, y_train, X_test, y_test, X_train_raw, y_train_raw):
    ...
    print(f"  CV Mean: {mean_cv:.3f} (±{std_cv:.3f})")
...
def save_model(model, scaler, feature_cols, ticker, model_name, cv_scores=None):
    ...
    model_data = {
        ...
        'cv_scores': cv_scores  # Save CV results
    }
...
if __name__ == "__main__":
    ...
    tickers = ['AAPL', 'MSFT', 'GOOGL', 'AMZN']  # No TSLA
